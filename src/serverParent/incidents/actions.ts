...
export async function updateIncidents(incidentID: any) {
  try {
    const token = await getToken();
    const payload = {
    ...
    const response = await fetch(
      `${baseURL}api/health/acknowledge_incident_report/${incidentID}/`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
