...
export async function getRewards() {
  try {
    ...
    const response = await fetch(
      `${baseURL}api/rewards/?login_profile=${loginProfile}&dropdown=true`.toString(),
      {
        headers: {
...
export async function getRewardsById(rewardId: any) {
  ...
  try {
    const token = await getToken();
    const url = `${baseURL}api/rewards/${rewardId}/?login_profile=${loginProfile}&dropdown=true`;
...
export async function getRewardsByCourse(courseId: any) {
  ...
  try {
    const token = await getToken();
    const url = `${getUrlWithParams}&course=${Number(courseId)}&dropdown=true`;
...
export async function getStudentRewardByCourse(courseId: any) {
  ...
  try {
    const token = await getToken();
    const url = new URL(
      `${baseURL}api/rewards/optimized_list/?login_profile=${loginProfile}&course=${courseId}`
    );
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (response.headers.get("Content-Length") !== "0") {
        ...
        console.log("Response was OK but no content was returned.");
        return null; // Or an appropriate value indicating success but no content
      }
    } else {
      console.error("Error:", response.status, response.statusText);
    }
