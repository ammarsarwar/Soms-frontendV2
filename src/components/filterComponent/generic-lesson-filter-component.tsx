...
interface GenericLessonFilterComponentProps {
  selectedLesson: TLessonSchema | null;
  setSelectedLesson: React.Dispatch<React.SetStateAction<TLessonSchema | null>>;
  lessons: TLessonSchema[];
  isLessonLoading: boolean;
  setIsLessonLoading: React.Dispatch<React.SetStateAction<boolean>>;
}

interface ISelectGenericLessonSchema {
  lesson: string;
}

const GenericLessonFilterComponent: React.FC<
  GenericLessonFilterComponentProps
> = ({
  selectedLesson,
  lessons,
  isLessonLoading,
  ...
  //form
  const form = useForm<ISelectGenericLessonSchema>({
    mode: "onChange",
  });
  ...
  function getLessonByLessonId(LessonArray: TLessonSchema[], lessonId: string) {
    const result = LessonArray.find((lesson) => lesson.id === Number(lessonId));
    return result || null;
  }

  const onSubmit = async (values: ISelectGenericLessonSchema) => {
    setOpen(false);
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          <p>{selectedLesson ? selectedLesson.title : "Select lesson"}</p>
        </Button>
      </PopoverTrigger>
...
export default GenericLessonFilterComponent;
