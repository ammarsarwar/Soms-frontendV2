...
import {
  editQuizMarks,
  postQuizMarks,
} from "@/serverAcademics/grading/marking/action";
...
export function DataTableRowActions<TData>({
  ...
  //form logic
  const form = useForm<IEditMarksSchema>({
    mode: "onChange",
  });
  ...
  const editSubmit = async (values: IEditMarksSchema, quiz: TQuizSchema) => {
    const refinedData = {
      ...values,
      obtained_marks: Number(values.obtained_marks),
      quiz: quiz.grade_record,
    };
    // console.log(refinedData);
    const res = await editQuizMarks(refinedData);
    if (res === undefined) {
      toast.error("error updating marks this assessment");
      setIsSingleQuizEditing(false);
      setSingleQuizId(undefined);
    } else {
      toast.success("Marks has been successfully updated for this assessment");
      setIsSingleQuizEditing(false);
      setSingleQuizId(undefined);
    }
  };
  ...
  const markSubmit = async (
    ...
    const refinedData = {
      ...values,
      obtained_marks: Number(values.obtained_marks),
      student: gradingRow.id,
      quiz: quiz.id,
    };
    // console.log(refinedData);
    const res = await postQuizMarks(refinedData);
    if (res === undefined) {
      toast.error("error marking this assessment");
      setIsSingleQuizEditing(false);
      setSingleQuizId(undefined);
    } else {
      toast.success("Student has been successfully marked for this assessment");
      setIsSingleQuizEditing(false);
      setSingleQuizId(undefined);
    }
  };
